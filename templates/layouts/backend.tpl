		...
		<header class="app__header" role="banner">
			...
			{if $currentUser}
				<div class="app__headerActions">
					{* SOPORTE BUTTON - Check user roles *}
					{assign var="showSupportButton" value=false}
					{assign var="userRoles" value=$currentUser->getAllRoles()}
					{foreach from=$userRoles item=role}
						{if $role->getRoleId() == 1 || $role->getRoleId() == 16}
							{* ROLE_ID_SITE_ADMIN = 1, ROLE_ID_MANAGER = 16 *}
							{assign var="showSupportButton" value=true}
							{break}
						{/if}
					{/foreach}
					...
					{if $showSupportButton}
						<div class="app__headerAction">
							<a href="https://desk.paideiastudio.net/helpdesk/soporte-tecnico-3" target="_blank" class="support-header-btn" title="Soporte Técnico">
